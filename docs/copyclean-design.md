...
then the message is fully saved. So there is no flag and no hook.
...
ClaudeCopy/
  ...
  scripts/install-ccopy.sh          # install `ccopy` on PATH for !ccopy + hotkeys
  hooks/hooks.json                  # intentionally empty (no hooks; see above)
